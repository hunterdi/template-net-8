...
namespace Core.Common.Providers
{
    public sealed class BlobStorageService : IBlobStorageService
    {
        ...
        {
            var tenantSettings = this._tenantService.GetTenantSettings();
        ...
        private async Task<BlobContainerClient> GetBlobContainer()
        {
            try
            {
                BlobContainerClient container = _blobServiceClient
                                .GetBlobContainerClient(this.GetStorage().Container);
            ...
            {
                _log.LogError($"Cannot find blob container: {this.GetStorage().Container} - error details: {ex.Message}");
                throw;
            }
        }
        ...
        public string GenerateSasTokenForContainer()
        {
            ...
            builder.BlobContainerName = this.GetStorage().Container;
